...
#define SONOBUS_SCHEME "flowbasketstudioliteaudio"
    ...
    : AudioProcessorEditor (&p), processor (p),  sonoSliderLNF(13), smallLNF(14), teensyLNF(11), panSliderLNF(12)
{
    ...
    mTitleLabel = std::make_unique<Label>("title", TRANS("Studio Lite"));
    ...
    mConnectButton->setColour(TextButton::buttonColourId, Colour::fromRGB((uint8_t) 18, (uint8_t) 145, (uint8_t) 239));
    mConnectButton->setColour(TextButton::buttonOnColourId, Colour::fromFloatRGBA(0.6, 0.4, 0.6, 0.6));
    mConnectButton->setColour(SonoTextButton::outlineColourId, Colour::fromString("transparent"));
...
void SonobusAudioProcessorEditor::buttonClicked (Button* buttonThatWasClicked)
{
    if (buttonThatWasClicked == mConnectButton.get() || buttonThatWasClicked == mAltConnectButton.get()) {
        ...
        if (processor.isRecordingToFile()) {
            ...
            String filename = (currGroup.isEmpty() ? "StudioLiteSession" : currGroup) + String("_") + Time::getCurrentTime().formatted("%Y-%m-%d_%H.%M.%S");
...
void SonobusAudioProcessorEditor::showSaveSettingsPreset()
{
    ...
    if (FileChooser::isPlatformDialogAvailable())
    {
        File recdir; // = File::getSpecialLocation(File::userDocumentsDirectory).getChildFile("StudioLite Setups");
