    ...
    if (info == nullptr)
    {
        if (showAlertWindows)
            AlertWindow::showMessageBoxAsync (AlertWindow::WarningIcon,
                                              ...
                                              "Failed to communicate with the Studio Lite update server.\n"
                                              "Please try again in a few minutes.\n\n"
                                              "If this problem persists you can download the latest version of Studio Lite from flowbasket.com");
    ...
    if (! info->isNewerVersionThanCurrent())
    {
        if (showAlertWindows)
            AlertWindow::showMessageBoxAsync (AlertWindow::InfoIcon,
                                              ...
                                              "Your Studio Lite version is up to date.");
        return;
    }
...
public:
    ...
    {
        titleLabel.setText ("Studio Lite Audio version " + newVersion, dontSendNotification);
        titleLabel.setFont ({ 15.0f, Font::bold });
        titleLabel.setJustificationType (Justification::centred);
        addAndMakeVisible (titleLabel);

        contentLabel.setText ("A new version of Studio Lite Audio is available - would you like to download it?", dontSendNotification);
                                                       ...
                                                       const String& releaseNotes)
    {
        ...
        options.dialogTitle = "Download Studio Lite Audio version " + newVersionString + "?";
...
private:
    ...
    Result install (const MemoryBlock& data)
    {
        ...
        if (zip.getNumEntries() == 0)
            return Result::fail ("The downloaded file was not a valid Studio Lite Audio release file!");
