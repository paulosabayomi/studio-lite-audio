...
//    mConnectTab->addTab(TRANS("RECENTS"), Colour::fromFloatRGBA(0.1, 0.1, 0.1, 1.0), mRecentsContainer.get(), false);
    mConnectTab->addTab(TRANS("CREATE OR CONNECT GROUP"), Colour::fromFloatRGBA(0.1, 0.1, 0.1, 1.0), mServerConnectViewport.get(), false);
...
//    mConnectTab->addTab(TRANS("DIRECT"), Colour::fromFloatRGBA(0.1, 0.1, 0.1, 1.0), mDirectConnectContainer.get(), false);
    ...
    mDirectConnectDescriptionLabel = std::make_unique<Label>("dirconndesc", TRANS("Connect directly to other instances of Studio Lite on your local network with the local address that they advertise. This is experimental, using a private group is recommended instead, and works fine on local networks."));
...
{
    String message = TRANS("Share this link with others to connect with Studio Lite:") + " \n";
    ...
    String urlstr1;
    urlstr1 << String("flowbasketstudioliteaudio://") << hostport << String("/");
    URL url(urlstr1);
    URL url2("http://studiolite.flowbasket.com/stalaunch");
...
void ConnectView::buttonClicked (Button* buttonThatWasClicked)
{
    if (buttonThatWasClicked == mDirectConnectButton.get()) {
        ...
        // @TODO: open studiolite video meeting app
        juce::URL deepLink("flowbasketstudiolite://join-session?id="+mServerGroupEditor->getText().trim());
...
bool ConnectView::attemptToPasteConnectionFromClipboard()
{
    ...
    if (clip.isNotEmpty()) {
        ...
        String urlpart = clip.fromFirstOccurrenceOf("flowbasketstudioliteaudio://", true, true);
        if (urlpart.isNotEmpty()) {
            ...
            if (url.isWellFormed()) {
                DBG("Got good studiolite URL: " << urlpart);
            ...
            urlpart = clip.fromFirstOccurrenceOf("http://studiolite.flowbasket.com/sblaunch?", true, false);
            if (urlpart.isEmpty()) urlpart = clip.fromFirstOccurrenceOf("https://studiolite.flowbasket.com/sblaunch?", true, false);
            ...
            if (urlpart.isNotEmpty()) {
                ...
                if (url.isWellFormed()) {
                    DBG("Got good http studio lite URL: " << urlpart);
...
bool ConnectView::handleSonobusURL(const URL & url)
{
    ...
    if (url.getScheme() == "flowbasketstudioliteaudio") {
...
void ConnectView::updateServerFieldsFromConnectionInfo()
{
    if (currConnectionInfo.serverPort == DEFAULT_SERVER_PORT) {
        mServerHostEditor->setText("aoo.sonobus.net", false);
